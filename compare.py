...
def main(csv_file: list[str]):
    ...
    csv_files = [
        ...
        for filename in os.listdir(directory_name) if ".csv" in filename
    ] if csv_file == None else csv_file

    for filename in csv_files:
        p = subprocess.run(['./small_buffer_read_csv.out', filename, '1'])
        if p.returncode != 0:
            exit(1)
...
def compare_two_result(orig_csv, interpreted_csv, precision_threshold):
    ...
    for i in range(len(features)):
        ...
        idx = abs(combined_view['orig'] - combined_view['interpreted']) < precision_threshold
        if idx.all() == False:
